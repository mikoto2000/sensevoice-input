...
UIA入力欄判定は約500ms間隔。未対応アプリはARMEDにならない場合がある。認識中はマイクを停止するため連続会話の全取り込みは行わない。30秒で区切った場合も同様に認識中の音声は欠落する。環境音、テレビ、他人の声の誤検出はあり得る。話者分離・エコーキャンセル・無限連続録音は未実装。音声はメモリのみだが、OS・native内部bufferの完全消去までは保証しない。
