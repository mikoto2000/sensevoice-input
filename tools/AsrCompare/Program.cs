...
if(args.Length<4) { Console.Error.WriteLine("Usage: AsrCompare <whisper-dir> <sensevoice-dir or -> <CUDA|CPU> <wav> [wav...]. Prints recognized text for explicit development comparison only."); return 2; }
using var cts=new CancellationTokenSource(); Console.CancelKeyPress+=(_,e)=>{e.Cancel=true;cts.Cancel();};
var provider=Enum.Parse<RecognitionBackend>(args[2],true);
using var whisper=new WhisperOnnxRecognitionService(args[0],provider,Console.Error.WriteLine);
using var sense=args[1]=="-" ? null : new SenseVoiceRecognitionService(()=>args[1],()=>RecognitionBackend.CPU);
foreach(var file in args.Skip(3))
{
    ...
    try
    {
        foreach(var (engine,recognizer) in new (string,ISpeechRecognitionService?)[] {("Whisper cold/current",whisper),("Whisper reused",whisper),("SenseVoice",sense)})
        {
