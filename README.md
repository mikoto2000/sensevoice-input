...
PTT・自動入力とも、文字を送る直前に入力先のIMEへOFFを要求し、入力後もOFFを維持します。直接入力・貼り付けの両方式に適用します。IME応答のタイムアウトやOFF失敗を検出した場合は入力を中止します。IMM互換窓を持たない入力先では切替できない場合があります。入力言語・キーボード配列自体は変更しません。
