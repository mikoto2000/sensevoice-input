...
キー問題の診断時だけ `--keyboard-diagnostics` を追加できます。このモードは録音・文字入力を無効にし、英数/Caps と関連する IME モードキーの message / virtual-key / scan / flags を最大 2000 イベントだけ診断ログに出します。通常文字キーや入力文字列は記録しません。`message=0xFF` は Raw Input、`0xFE` は Raw Input 取得エラー（flags に Win32 エラー番号）。比較のため OEM の IME キーを通過させるので、診断中は英数キー本来の入力モード切り替えが起こり得ます。通常モードに戻すには引数を外して再起動します。
...
- JIS リピートで届く「解放→押下」を1回の長押しとして扱うため、解放を50ms保留します。さらに、JIS 英数の最終解放イベント自体が来ない環境では、リピートが途切れたことで停止を推定します。Windows のリピート設定から計算し、通常設定の長押しでは約0.1～0.2秒、短押しでは初回リピート待機分だけ長くなります。極端に遅いリピート設定では最大約1.9秒。待機中の再押下は同じ発話にまとめます。リピートを無効にする Filter Keys 等の構成では保持時間を正確に検出できません。リピート設定変更後は再起動してください。
