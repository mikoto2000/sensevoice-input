...
internal static class InputMethodControl
{
    ...
    internal static void Disable(nint target)
    {
        ...
        // Leave IME off after dictation; do not toggle or restore it.
    }
    ...
    private static nuint Send(nint ime, nuint command)
    {
        // Bound waits on foreign processes, and don't dispatch reentrant input while waiting.
        if (SendMessageTimeout(ime, 0x0283, command, 0, 0x0003, 250, out var result) == 0)
            throw new Win32Exception(Marshal.GetLastWin32Error(), "入力先のIMEが応答しません。IMEを手動でオフにして再試行してください。");
        return result;
    }
